...
\textbf{Nawigacja po sekcjach:} Strona musi umożliwiać użytkownikowi płynne przechodzenie po konkretnych sekcjach strony (Home, Our Story, Services, Jobs, Contact) poprzez kliknięcie w odpowiedni załącznik prowadzący do wspomnianych sekcji.
...
\textbf{Animacja powitalna:} Podczas ładowania strony głównej, strona automatycznie ma odtworzyć prostą animację powitalną, aby przykuć uwagę użytkownika.
...
\textbf{Technologie:} Strona zgodnie z przyjętymi wytycznymi wykonana ma być przy użyciu czystego \texttt{HTML5} oraz \texttt{CSS} bez zewenętrznych bibliotek, frameworków (np. Bootstrap, React itd.) oraz języka \texttt{JavaScript}.
...
\textbf{Wydajność:} Strona powinna charakteryzować się krótkim czasem ładowania poprzez brak ciężkich bibliotek i zewnętrznych skryptów.
